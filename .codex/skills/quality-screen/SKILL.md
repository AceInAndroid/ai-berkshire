---
name: quality-screen
description: Screen companies or themes for quality using hard exclusion criteria, financial evidence, and concise Chinese output.
---

# Quality Screen

This is the Codex-adapted derivative of `skills/quality-screen.md`. Treat the original file as the Claude command source and this file as the executable Codex skill for this local OMX/Codex profile.
...
对 the user-supplied company, ticker, industry, or topic 执行去劣指标筛选，快速排除不符合一流公司标准的标的。
...
- 如果输入是行业/市场/主题 → **批量模式**，先执行以下操作：
  1. 用 current web research 搜索该行业/市场/主题下的主要上市公司
...
为每家公司启动独立后台Agent，使用 current web research 搜索以下数据：
