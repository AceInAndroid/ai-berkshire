...
description: Research a private company in Chinese using limited-information discipline, source confidence labels, and first-principles analysis.
---

# Private Company Research

This is the Codex-adapted derivative of `skills/private-company-research.md`. Treat the original file as the Claude command source and this file as the executable Codex skill for this local OMX/Codex profile.
...
对 the user-supplied company, ticker, industry, or topic 进行团队化深度研究分析。专为蚂蚁集团、小红书、SpaceX、Stripe 等未上市公司设计。
...
Team Lead 负责统筹。如果当前 Codex 会话允许并且用户明确需要并行研究，可以把以下 6 个任务作为独立 subagent lane 并行执行；否则按同样顺序串行执行，并在最终报告元信息中注明 `execution_mode: sequential`.
...
**研究方法**：
1. 使用 current web research 搜索最新公开信息，每个维度至少搜索3-5次，用不同关键词组合
2. 搜索关键词策略：
