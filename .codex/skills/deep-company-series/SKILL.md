...
description: Create an eight-part Chinese deep company series with strict fact checking, revision handling, and publication-quality structure.
---

# Deep Company Series

This is the Codex-adapted derivative of `skills/deep-company-series.md`. Treat the original file as the Claude command source and this file as the executable Codex skill for this local OMX/Codex profile.
...
为 the user-supplied company, ticker, industry, or topic 撰写一个 8 篇深度长文系列，发布在公众号/视频号等公开渠道。**核心 IP 不是"会写"，而是"会改"——99% 的财经文章在违反本 skill 的事实核查标准**。
...
rg -n "<personal-name>|/Users/|<internal-codename>|<private-position>" reports/ | head
```
...
如果用户说"X 数据不对"，先用 shell/Read 找原始数据交叉验证：
...
- 推送前必须用 `rg` 扫描用户个人姓名、公司花名、本机绝对路径、内部项目代号、未公开持仓信息等隐私字段；发现后先删除或脱敏再发布。
- 公开署名按用户多层身份策略，不混用
