...
description: Apply a Buffett-style Chinese buy-before checklist to one or more companies with hard rejection rules and data checks.
---

# Investment Checklist

This is the Codex-adapted derivative of `skills/investment-checklist.md`. Treat the original file as the Claude command source and this file as the executable Codex skill for this local OMX/Codex profile.
...
对 the user-supplied company, ticker, industry, or topic 执行巴菲特价值投资买入前 Checklist 分析。
...
从 the user-supplied company, ticker, industry, or topic 中解析出所有公司名称/代码。对每家公司确定：
- 公司全称、股票代码、上市交易所
...
使用 Codex subagent or sequential research lane 工具为**每家公司**启动独立的后台 Agent 进行数据收集（所有公司同时并行启动），每个Agent负责收集：
