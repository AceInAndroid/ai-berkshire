...
Longbridge 在本仓库中属于**第三方聚合数据源**，不是计算工具，也不是原始财报的替代品：

1. 多个 Longbridge 工具仍只算**一个来源**，不能互相构成双源验证。
2. 原始年报、交易所公告、SEC/HKEX/巨潮披露的优先级高于 Longbridge 汇总数据。
3. PE、PB、ROE、市值、FCF Yield 等计算必须继续使用 `tools/financial_rigor.py` 验算。
4. 所有研究流程默认只允许调用 Longbridge 的只读市场与公司工具；禁止下单、撤单、改单、定投、提醒、自选股和社区写入。
5. 除非用户明确要求分析本人组合，否则不得读取账户余额、持仓、订单、银行卡、入金或出金信息。

推荐链路：

```text
Longbridge MCP 结构化取数
  → 原始披露或第二独立来源交叉验证
  → tools/financial_rigor.py 精确计算
  → tools/report_audit.py 报告抽检准出
```
...
Longbridge MCP 可用时，先按 `skills/longbridge-data.md` 生成结构化数据底稿。随后对每个财务指标（收入、净利润、毛利率、经营现金流、资产负债率等），分别从**Longbridge或来源1**与**另一个独立来源**取数。
